// // we make some helper functions to upload videos to BunnyCDN
...
export const withErrorHandling = <T, A extends unknown[]>(
    // wrap api calls with this function to handle errors
  fn: (...args: A) => Promise<T>
) => {
...
export const createIframeLink = (videoId: string) =>
  `https://iframe.mediadelivery.net/embed/559823/${videoId}?autoplay=true&preload=true`;
